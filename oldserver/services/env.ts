...
export async function moveWithCollision(dx: number, dy: number) {
    ...
    const objectHere = findObjectAt(snapshot, nx, ny);
    if (objectHere) {
        agentMemory.rememberObject(objectHere, { x: nx, y: ny });
    }
...
type MoveToTargetOptions = {
    onStep?: (step: {
        ...
        result: Awaited<ReturnType<typeof moveWithCollision>>;
    }) => void | Promise<void>;
};

export async function moveToTarget(targetX: number, targetY: number, options?: MoveToTargetOptions) {
    const env = await ensureEnv();
    if (!env.player) {
        return { ok: false, reason: "Player does not exist.", pos: null, steps: [] as Array<{ x: number; y: number }> };
    }

    const goal = { x: Math.trunc(targetX), y: Math.trunc(targetY) };
    if (!inBounds(env, goal.x, goal.y)) {
        return { ok: false, reason: "Target out of bounds.", pos: env.player, steps: [] as Array<{ x: number; y: number }> };
    }

    if (env.player.x === goal.x && env.player.y === goal.y) {
        return { ok: true, pos: env.player, steps: [] as Array<{ x: number; y: number }> };
    }

    const path = planPathBFS(env, env.player, goal);
    if (!path) {
        return { ok: false, reason: "No path to target.", pos: env.player, steps: [] as Array<{ x: number; y: number }> };
    }

    const visited: Array<{ x: number; y: number }> = [];
    let current = env.player;
    ...
    for (let i = 1; i < path.length; i++) {
        ...
        if (!result.ok || !result.pos) {
            return {
                ok: false,
                reason: result.reason ?? "Movement interrupted.",
                pos: result.pos ?? current,
                steps: visited,
            };
        }
        current = { x: result.pos.x, y: result.pos.y };
    ...
    return { ok: true, pos: current, steps: visited };
}
